...
const UsdJpyChart = () => {
  ...
  const spread = 0.02; // Adjusted typical spread for USD/JPY (around 2 pips)
  const buyPrice = price ? (price + spread).toFixed(3) : '--';  // JPY pairs usually 3 decimals
  const sellPrice = price ? (price - spread).toFixed(3) : '--';
            ...
            <div>
              ...
              <div className="d-flex justify-content-between align-items-center px-2 mb-3">
                ...
                <div className="d-flex flex-column text-start">
                  ...
                  <span className="fw-bold text-dark">{low ? low.toFixed(3) : '--'}</span>
                </div>

                {/* High */}
                <div className="d-flex flex-column text-end">
                  <small className="text-uppercase text-secondary fw-medium">High</small>
                  <span className="fw-bold text-dark">{high ? high.toFixed(3) : '--'}</span>
                </div>
              </div>
