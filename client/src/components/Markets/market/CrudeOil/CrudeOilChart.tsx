...
const CrudeOilChart: React.FC = () => {
  const [price, setPrice] = useState<number | null>(6691.4); // Fallback default
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.symbol === 'USOILUSD' && data.price) {
  ...
  return (
    <div className="w-100 bg-[#fff7ee] py-5">
      <div className="container bg-white rounded-3 shadow border border-orange-200 overflow-hidden">
        {/* Header */}
        <header
          className="bg-orange-100 text-orange-700 text-center fw-bold py-3 fs-4 rounded-top-3"
        >
          US Crude Oil Live Chart (USOILUSD)
        </header>
        ...
        <div className="row g-0">
          ...
          <section className="col-lg-9 p-4" style={{ minHeight: 600 }}>
            <AdvancedRealTimeChart
              ...
              style="1"
              container_id="crude_oil_chart_container"
            />
          </section>
